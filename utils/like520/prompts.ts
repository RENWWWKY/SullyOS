...
4. **锚点剧本**（anchors）：**7-10 个锚点**（**少于 7 个就太短了，让 ${userName} 没玩够**）。**这是养成游戏的核心机制**——
   ...
   **陪画画 ✏️**（暴露真心型）：
   ...
   - \`dialogue\`：**对白行数组**。每条数组项 = 一句你说的话 = 一个独立气泡，按顺序推进。
     ...
     - 节奏建议：开局一句轻反应/接住动作（生活化）→ 中段 2-3 句逐渐暴露真心/泄露看见/颠倒关系（这是 anchor 的母题主体）→ 收尾 1-2 句捂嘴/挪开/装没说（让真心话被自己打断的小段子）
     - **至少要有一处"让 ${userName} 心头一震"** 的话——具体到 ta 自己的某个姿态/某个时刻/某个 ta 以为没人看见的状态。短句、停顿、省略号、破折号是工具。
     - 不要纯客气话铺满。
